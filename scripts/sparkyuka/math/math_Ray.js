import { Plane } from "./math_Plane"

import {ZERO, MINUS_ONE, ZERO_POINT, ONE, copyPoint, vectorEquals, copyVector} from "./math_common"
const R = require('Reactive');
const DEFAULT_NORMAL = R.point(ZERO, ZERO, ONE);
...
class Ray {
	...
	*
	* @param {PointSignal} origin - The origin of the ray.
	* @param {VectorSignal} direction - The direction of the ray.
	*/
	constructor( origin = ZERO_POINT, direction = ZERO_POINT ) {
	...
	*/
	copy( ray ) {
		...
		this.origin = copyPoint( ray.origin );
		this.direction = copyVector( ray.direction);
	...
	*
	* @param {ScalarSignal} t - A scalar value representing a position on the ray.
	* @return {PointSignal} The result vector.
	*/
	at( t ) {

		// t has to be zero or positive
		return R.add(R.mul(this.direction, t), this.origin);
	...
	* to the given 3D vector. This assumes there is already an intersection via intersectsPlane
	*
	...
	*/
	intersectPlane( plane ) {
		...
		const denominator = R.dot(plane.normal, this.direction);

		const t = denominator.eq(ZERO).ifThenElse(
			plane.distanceToPoint( this.origin).eq(ZERO).ifThenElse(ZERO, MINUS_ONE),
			R.div( R.mul(R.add(R.dot(this.origin, plane.normal), plane.constant), MINUS_ONE), denominator )
		);

		return this.at(t);
		//t.ge(ZERO).ifThenElse(this.at( t ), null); // <- tricky to get this to work properly
	}
	...
	*
	* @param {Plane} plane - A plane.
	* @return {BoolSignal} Whether there is an intersection or not.
	*/
	intersectsPlane( plane ) {
		...
		return R.or(distToPoint.eq(ZERO), R.dot(plane.normal, this.direction).lt(ZERO));
	}
	...
	*/
	equals( ray ) {
		return R.and(vectorEquals(ray.origin, this.origin), vectorEquals(ray.direction, this.direction));
	}
